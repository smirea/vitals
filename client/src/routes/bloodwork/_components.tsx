...
import { memo, useCallback, useEffect, useLayoutEffect, useMemo, useRef, type CSSProperties, type ChangeEvent } from 'react'
...
function ScopedVitals(props: { children: React.ReactNode }) {
    ...
    return (
        <VitalsScope style={getThemeVars(token)}>
            {props.children}
        </VitalsScope>
    )
}
                        ...
                        return (
                            <article key={item.key} className='vitals-meaningful-change-item'>
                                ...
                                <div className='vitals-meaningful-change-meta'>
                                    ...
                                    {relativeDelta ? <span className='vitals-meaningful-change-delta'>Value Δ {relativeDelta}</span> : null}
                                    {rangeDelta ? <span className='vitals-meaningful-change-delta'>Range drift {rangeDelta}</span> : null}
                                </div>
                            </article>
                        )
                    })}
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {visibleSeries.map(item => {
                                                    const cell = item.valuesBySourceIndex[source.index]
                                                    const displayLabel = item.unitLabel ? `${item.label} (${item.unitLabel})` : item.label
                                                    const displayValue = cell?.display ?? '--'
                                                    const rangeLabel = cell?.rangeCaption?.trim()
                                                    const valueWithRange = rangeLabel ? `${displayValue} (${rangeLabel})` : displayValue
