...
export function formatCell(measurement: BloodworkMeasurementRecord): MeasurementCell {
	...
	const rangeBandLeft = hasBand
		? Math.min(rangeVisualization?.minPosition ?? 0, rangeVisualization?.maxPosition ?? 0)
		: 0;
	const rangeBandWidth = hasBand
		? Math.abs((rangeVisualization?.maxPosition ?? 0) - (rangeVisualization?.minPosition ?? 0))
		: 0;
...
export function resolveSeriesUnitLabel(
	cells: Array<MeasurementCell | undefined>,
): string | undefined {
	const normalizedUnits = new Set(
		cells.map(cell => cell?.unit?.trim()).filter((unit): unit is string => Boolean(unit)),
	);
...
export function getRowDefaultRange(
	cells: Array<MeasurementCell | undefined>,
...
export function getRowObservedBounds(
	cells: Array<MeasurementCell | undefined>,
): { min: number; max: number } | null {
	const values = cells
...
export function normalizeCellForChart({
	...
	if ((rangeMin === null || rangeMax === null || rangeMax <= rangeMin) && defaultRange) {
...
export function getChartSources({
	...
	return visibleSources
		.filter(source =>
			selectedRows.some(row => hasCellNumericValue(row.valuesBySourceIndex[source.index])),
		)
		.sort((left, right) => {
...
export function getTableRows({
	...
	const nonFavoriteRows = filteredMeasurementRows.filter(
		row => !starredMeasurementSet.has(row.key),
	);
...
export function getCategoryOverviewByLatestAcrossAllLabs({
	...
	allMeasurementRows.forEach(row => {
		const latestSourceEntry = latestSources.find(source =>
			hasCellDisplayValue(row.valuesBySourceIndex[source.source.index]),
		);
...
export function getSixMonthMeaningfulChanges({
	...
	allMeasurementRows.forEach(row => {
		...
		const relativeDeltaPercent =
			latestNumeric !== null && previousNumeric !== null
				? (Math.abs(latestNumeric - previousNumeric) / Math.max(Math.abs(previousNumeric), 1e-6)) *
					100
				: null;
		...
		const normalizedRangeDeltaPercent =
			latestNormalizedPosition !== null && previousNormalizedPosition !== null
...
export function getChartSeries({
	...
	selectedRows.forEach(row => {
		...
		const normalizedValuesBySourceIndex = Array.from(
			{ length: row.valuesBySourceIndex.length },
			() => null as number | null,
		);
		const outOfRangeBySourceIndex = Array.from(
			{ length: row.valuesBySourceIndex.length },
			() => false,
		);
		...
		for (const source of chartSources) {
			...
			outOfRangeBySourceIndex[source.index] =
				isCellOutsideReferenceRange(cell) ||
