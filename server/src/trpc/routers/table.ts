...
type TableFilterConditionForColumn<TColumnName extends string, TColumn extends AnyColumn> =
	| {
			column: TColumnName;
			operator: 'eq';
...
function buildWhereSchema<TTable extends AnySQLiteTable>(
	table: TTable,
): z.ZodType<TableFilterCondition<TTable>[]> {
...
function buildOrderBySchema<TTable extends AnySQLiteTable>(
	table: TTable,
): z.ZodType<TableQueryInput<TTable>['orderBy']> {
...
function buildTableQuerySchema<TTable extends AnySQLiteTable>(
	table: TTable,
): z.ZodType<TableQueryInput<TTable>> {
	return z.object({
...
function buildOrderBy<TTable extends AnySQLiteTable>(
	...
	return orderBy.map(item =>
		item.direction === 'desc' ? desc(columns[item.column]) : asc(columns[item.column]),
	);
}
...
function createTableAccessRouter<TTable extends AnySQLiteTable>(table: TTable) {
	...
	return createRouter({
		...
		findOne: publicProcedure.input(querySchema.optional()).query(({ ctx, input }) => {
			const safeInput = input ?? {};
			const whereClause = buildWhereClause(table, safeInput.where);
			const orderBy = buildOrderBy(table, safeInput.orderBy);
			const offset = safeInput.offset ?? 0;
		...
		count: publicProcedure.input(querySchema.optional()).query(({ ctx, input }) => {
		...
		deleteMany: publicProcedure.input(deleteSchema).mutation(({ ctx, input }) => {
			const whereClause = buildWhereClause(table, input.where);
			if (!whereClause) {
...
const tableRouters = Object.fromEntries(
	Object.entries(appTables).map(([tableName, table]) => [
		tableName,
		createTableAccessRouter(table),
	]),
) as {
