...
const pillImageExtractionSchema = z.object({
    detected: z.boolean(),
    name: z.string().nullable().optional().describe('Product name only. Do not include serving size or supplement facts text.'),
    value: z.string().nullable().optional().describe('Serving amount only, such as "2" or "1 scoop". Do not include ingredient lists.'),
    unit: z.string().nullable().optional().describe('Serving unit only, such as "capsules", "softgels", "tablet", or "scoop".'),
...
function buildPillsPayload(args: {
    ...
    const componentMap = new Map<number, Array<{
        id: number;
        name: string;
        value: string | null;
        unit: string | null;
    }>>();
    const imageMap = new Map<number, Array<{
        id: number;
        fileName: string;
        dataUrl: string;
    }>>();
    const periodMap = new Map<number, Array<{
        id: number;
        startDate: string;
        endDate: string | null;
...
function getPillRecords(db: PillsReadDb, pillIds?: number[]) {
    ...
    const pillRows = (shouldFilterByIds
        ? pillRowsQuery.where(inArray(pills.id, pillIds))
        : pillRowsQuery)
    ...
    const componentRows = db.select().from(pillComponents)
...
export function searchPills(db: VitalsDatabase, input: z.infer<typeof pillSearchInputSchema>) {
    ...
    const whereClause = query.length === 0 ? null : or(
        like(pills.name, `%${query}%`),
        like(pills.name, `${query}%`),
    );
    ...
    const pillRows = queryBuilder
        .orderBy(asc(pills.name), asc(pills.id))
        .limit(input.limit)
        .all();

    return getPillRecords(db, pillRows.map(row => row.id));
}
...
export async function extractPillFromImages(input: z.infer<typeof pillImageExtractionInputSchema>) {
    ...
    if (!model.startsWith('google/gemini')) {
        throw new Error(`OPENROUTER_MODEL must be a Gemini model for pill image parsing. Received: ${model}`);
    }
    ...
    const parsedResponse = pillImageExtractionSchema.parse(parseJsonFromText(result.text)) as PillImageExtraction;
    return {
